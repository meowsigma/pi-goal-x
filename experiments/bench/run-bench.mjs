 ...
 * agent-free guard active, then writes:
 ...
 *        --experimental-strip-types experiments/bench/run-bench.mjs before
...
const specDir = path.join(benchDir, "..", "..", "specs", "2026-08-04-extension-review-plan");
...
console.log(`[bench] phase=${phase} — agent-free run (B8) over real extension functions`);
...
const jsonPath = path.join(benchDir, `baseline-${phase}.json`);
